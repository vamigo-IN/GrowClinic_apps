...
//
// Used for the site → tool handoff tokens (/api/intake → /api/handoff/:token).
// Tokens used to live only in process memory, so a container restart or
// redeploy inside the 15-minute window silently broke the visitor's prefill.
// With REDIS_URL set they survive restarts; without it (or while Redis is
// unreachable) the previous in-memory behaviour is used unchanged.
//
// Keys are namespaced `audit:<namespace>:<token>` (the platform Redis ACL only
// lets this app touch `audit:*`). Values expire server-side (PX) and are
// consumed atomically with GETDEL, so a token can be redeemed exactly once.
// ─────────────────────────────────────────────────────────────
...
function redis() {
    ...
    if (!client) {
        client = createClient({
            ...
            // Fail fast instead of queueing while disconnected — callers fall
            // back to memory rather than hanging a request.
...
async function putOnce(namespace, token, data, ttlMs) {
...
async function takeOnce(namespace, token) {
    const key = `${KEY_PREFIX}${namespace}:${token}`;
...
module.exports = { putOnce, takeOnce };
