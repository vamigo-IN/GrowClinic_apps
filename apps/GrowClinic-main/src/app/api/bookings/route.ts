...
export async function POST(req: Request) {
  try {
    ...
    const clinicType = text(body?.clinicType, 120);
    const challenge = text(body?.challenge, 5000);
    const preferredDate = text(body?.preferredDate, 40);
    const preferredTime = text(body?.preferredTime, 40);
    ...
    pushToCrm({
      ...
      phone,
      clinicName,
      source: "growclinic-booking",
      message: `${clinicType} | ${challenge}${preferredDate ? ` | Preferred: ${preferredDate} ${preferredTime || ""}` : ""}`,
    });
        ...
        html: `
          <div style="font-family: 'Inter', sans-serif; max-width: 600px; margin: 0 auto; line-height: 1.6; color: #1a1a1b; background-color: #ffffff;">
            ...
            <div style="padding: 40px; border: 1px solid #f0f0f1; border-top: none; border-radius: 0 0 16px 16px;">
              <h2 style="margin-top: 0; color: #111827; font-size: 20px; font-weight: 700;">Hi ${escapeHtml(name)},</h2>
              <p style="font-size: 16px; color: #4b5563;">Thank you for requesting a consultation with <strong>GrowClinic</strong>. Our team is excited to help you scale <strong>${escapeHtml(clinicName)}</strong>.</p>
              ...
              <div style="background-color: #f8fafc; padding: 25px; border-radius: 12px; border: 1px solid #e2e8f0; margin: 30px 0;">
                ...
                <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 20px;">
                  ...
                  <p style="margin: 5px 0 0 0; font-size: 16px; color: #3586ff; font-weight: 700;">
                    ${escapeHtml(preferredTime || 'Pending Call')}
                  </p>
                </div>
                
                <hr style="border: none; border-top: 1px solid #e2e8f0; margin: 20px 0;" />
                
                <p style="margin: 0; font-size: 14px; color: #64748b; margin-bottom: 5px;"><strong>Clinic Specialty:</strong> ${escapeHtml(clinicType)}</p>
                <p style="margin: 0; font-size: 14px; color: #64748b;"><strong>Current Challenge:</strong> "${escapeHtml(challenge)}"</p>
              </div>

              <p style="font-size: 15px; color: #4b5563;">Our growth expert will reach out to you at <strong>${escapeHtml(phone)}</strong> shortly to finalize the details and provide the meeting link.</p>
              
              <div style="margin-top: 40px; text-align: center;">
                <a href="https://www.growclinic.io/" style="background-color: #3586ff; color: #ffffff; padding: 14px 28px; border-radius: 10px; text-decoration: none; font-weight: 600; font-size: 15px; display: inline-block;">Explore Case Studies</a>
              </div>
