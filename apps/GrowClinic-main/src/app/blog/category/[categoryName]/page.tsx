...
export default async function CategoryPage({ params }: { params: Promise<{ categoryName: string }> }) {
  ...
  // Fetch all published posts, then filter in memory (handles case mismatches
  // like DB "Insights" vs URL "insights"). Guarded so a brief DB outage renders
  // an empty state instead of a 500 — a 5xx here can trip the host health check
  // and restart the app.
  let allPosts: any[] = [];
  try {
    ...
    console.error("CategoryPage: DB unavailable, rendering empty list.", e);
  }
    ...
    <main className="pt-28 pb-20 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/5 border border-primary/10 text-primary text-size-tiny text-weight-bold uppercase tracking-widest mb-8">
        ...
        {categories.length > 0 && (
          <div className="flex flex-wrap justify-center gap-2 mb-16 relative z-10">
            <Link href="/blog" className="px-5 py-2.5 rounded-full bg-white text-slate-500 text-size-tiny text-weight-bold uppercase tracking-widest cursor-pointer border border-slate-100 hover:bg-primary/5 hover:text-primary hover:border-primary/20 transition-all">
              All Articles
            </Link>
            {categories.map((cat) => {
              ...
              return (
                <Link 
                  ...
                  className={`px-5 py-2.5 rounded-full text-size-tiny text-weight-bold uppercase tracking-widest cursor-pointer transition-all ${
                    isActive 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredPosts.map((post) => (
            <article key={post.id} className="group relative bg-white rounded-[15px] border border-slate-100 overflow-hidden shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-500">
              <Link href={`/blog/${post.slug}`} className="absolute inset-0 z-[1]"><span className="sr-only">Read {post.title}</span></Link>
              
              <div className="block relative h-56 overflow-hidden pointer-events-none bg-slate-50">
                <Image
              ...
              <div className="p-8 relative">
                <div className="flex items-center gap-3 mb-5 pointer-events-none">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                    ...
                    <span suppressHydrationWarning>{new Date(post.publishedAt ?? post.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                  </span>
                ...
                <div className="flex items-center justify-between pt-6 border-t border-slate-50 pointer-events-none text-xs">
                  <span className="text-size-tiny text-weight-bold uppercase tracking-widest text-slate-400">Read article</span>
        ...
        {filteredPosts.length === 0 && (
          <div className="text-center py-24 bg-gradient-to-br from-slate-50 to-white rounded-[15px] border border-dashed border-slate-200 relative z-10">
